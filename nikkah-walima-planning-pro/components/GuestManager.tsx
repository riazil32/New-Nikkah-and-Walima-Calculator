  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-3 sm:p-4 mb-4">
        ...
        {addMode === 'individual' && (
          <div>
            {/* Name + Role: 50/50 on mobile, flexible on desktop */}
            <div className="flex gap-2 mb-3">
              <input
                ...
                className="w-1/2 md:flex-1 min-w-0 px-4 py-3 bg-slate-50 dark:bg-slate-700 border-2 border-transparent focus:border-blue-400 rounded-xl transition-all outline-none font-medium text-slate-800 dark:text-white placeholder:text-slate-400"
              />
              <select
                value={individualForm.role}
                onChange={(e) => setIndividualForm(prev => ({ ...prev, role: e.target.value as GuestRole }))}
                className="w-1/2 md:w-48 px-3 py-3 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl outline-none font-medium text-slate-800 dark:text-white text-sm"
              >
              ...
              return (
                <div key={group.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border-2 border-violet-200 dark:border-violet-800/50 overflow-hidden">
                  ...
                  <div
                    ...
                    className={`w-full px-4 py-3 flex items-center justify-between bg-violet-50 dark:bg-violet-900/20 transition-colors ${!isEditingThisGroup ? 'hover:bg-violet-100 dark:hover:bg-violet-900/30 cursor-pointer' : ''}`}
                  >
                        ...
                        {isEditingThisGroup ? (
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              autoFocus
                              className="flex-1 px-2 py-1 bg-white dark:bg-slate-700 border-2 border-blue-400 rounded-lg outline-none font-bold text-slate-800 dark:text-white text-sm"
                            />
                            <button
                              onClick={(e) => { e.stopPropagation(); saveGroupName(); }}
                    ...
                    {!isEditingThisGroup && (
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); setDeleteConfirm({ type: 'group', id: group.id, name: group.name, memberCount: members.length }); }}
                  ...
                  {/* Group Members - Collapsible Content */}
                  {isExpanded && (
                    <>
                      ...
                      <button
                        onClick={() => handleAddMemberToFamily(group.id)}
                        className="w-full py-2.5 flex items-center justify-center gap-2 text-sm font-medium text-violet-600 dark:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-900/20 border-t border-dashed border-violet-200 dark:border-violet-800/50 transition-colors"
                      >
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-800 dark:text-white">Delete {deleteConfirm.type === 'group' ? 'Family' : 'Guest'}?</h3>
            ...
            <div className="mb-6 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
              ...
              {deleteConfirm.type === 'group' && deleteConfirm.memberCount && (
                <p className="text-sm text-red-600 dark:text-red-400 mt-1">
                  ⚠️ This will delete {deleteConfirm.memberCount} member{deleteConfirm.memberCount > 1 ? 's' : ''}
                </p>
              )}
            </div>
